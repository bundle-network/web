...
(function(win, doc, undefined) {
  ...
  RG.Registry.store['__rgraph_event_listeners__'] = [];
  RG.Background = {};
  RG.background = {};
  RG.objects = [];
  RG.Resizing = {};
  ...
  RG.ISOLD = RGraph.ISIE6 || RGraph.ISIE7 || RGraph.ISIE8;
  ...
  RG.getScale2 = function(obj, opt) {
    ...
    if (!max) {
      ...
      for (var i = 0; i < numlabels; ++i) {
        scale.labels.push(
          RG.number_format(obj, ((i + 1) / numlabels * (topValue - min) + min).toFixed(decimals), units_pre, units_post)
        );
      ...
      for (var i = 0; i < numlabels; ++i) {
        scale.labels.push(
          RG.numberFormat(obj, ((i + 1) / numlabels * (max - min) + min).toFixed(decimals), units_pre, units_post)
        );
  ...
  RG.Background.draw = RG.background.draw = RG.background.Draw = function(obj) {
    ...
    if (typeof prop['chart.title.yaxis'] == 'string' && prop['chart.title.yaxis'].length) {
      ...
      if (
        prop['chart.title.yaxis.align'] == 'right' ||
        prop['chart.title.yaxis.position'] == 'right' ||
        (obj.type === 'hbar' &&
          prop['chart.yaxispos'] === 'right' &&
          typeof prop['chart.title.yaxis.align'] === 'undefined' &&
          typeof prop['chart.title.yaxis.position'] === 'undefined')
      ) {
        angle = 90;
        yaxis_title_pos = prop['chart.title.yaxis.pos']
          ? ca.width - gutterRight + prop['chart.title.yaxis.pos'] * gutterRight
    ...
    if (bgcolor) {
      ...
      co.fillRect(
        ...
        ca.height - gutterTop - gutterBottom
      );
    }
        ...
        if (prop['chart.background.grid.autofit']) {
          if (prop['chart.background.grid.autofit.align']) {
            ...
            if (obj.type === 'line') {
              ...
              (obj.type === 'bar' || obj.type === 'scatter') &&
              ((prop['chart.labels'] && prop['chart.labels'].length) || obj.type === 'bar')
            ) {
              var len = (prop['chart.labels'] && prop['chart.labels'].length) || obj.data.length;
              obj.set({ backgroundGridAutofitNumvlines: len });
          ...
          if (coords && coords.length > 0) {
            ...
            if (obj.type === 'bar') {
              ...
              if (
                ...
                if (
                  text_y < 5 &&
                  (typeof labels_processed[i] === 'string' || typeof labels_processed[i][3] === 'undefined')
                ) {
                  text_y = y + 5 + length;
                  var valign = 'top';
                }
            ...
            co.beginPath();
            co.fillStyle =
              typeof labels_processed[i] === 'object' && typeof labels_processed[i][1] === 'string'
            ...
            RG.text2(obj, {
              ...
              text:
                typeof labels_processed[i] === 'object' && typeof labels_processed[i][0] === 'string'
              ...
              bounding: true,
              'bounding.fill':
                typeof labels_processed[i] === 'object' && typeof labels_processed[i][2] === 'string'
    ...
    if (xaxis) {
      if (xaxispos === 'center') {
        RG.path2(
          ...
          gutterLeft,
          gutterTop + halfGraphArea,
          gutterLeft + offsetx,
          gutterTop + halfGraphArea - offsety,
          ca.width - gutterRight + offsetx,
          gutterTop + halfGraphArea - offsety,
          ca.width - gutterRight,
          gutterTop + halfGraphArea
        );
      } else {
        if (obj.type === 'hbar') {
  ...
  RG.draw3DYAxis = function(obj) {
    ...
    RG.path2(
      ...
      gutterTop - offsety,
      x + offsetx,
      ca.height - gutterBottom - offsety,
      x,
      ca.height - gutterBottom
    );
  };
  ...
  RG.hideZoomedCanvas = RG.HideZoomedCanvas = function() {
    ...
    if (typeof RG.zoom_background === 'object') {
      setTimeout(
        "RGraph.zoom_background.style.display = 'none'",
        prop['chart.zoom.fade.out'] ? frames * interval + 10 : 0
      );
    }
  };
  ...
  RG.OR.remove = RG.OR.Remove = function(obj) {
    ...
    for (var i = 0; i < RG.ObjectRegistry.objects.byCanvasID.length; ++i) {
      if (
        RG.ObjectRegistry.objects.byCanvasID[i] &&
  ...
  RG.text2 = RG.Text2 = function(obj, opt) {
    function domtext() {
      ...
      var cacheKey =
        ma.abs(parseInt(opt.x)) +
        '_' +
        ma.abs(parseInt(opt.y)) +
        '_' +
        String(opt.text).replace(/[^a-zA-Z0-9]+/g, '_') +
        '_' +
        obj.canvas.id;
      if (!ca.rgraph_domtext_wrapper) {
        ...
        wrapper.style.overflow =
          obj.properties['chart.text.accessible.overflow'] != false &&
          obj.properties['chart.text.accessible.overflow'] != 'hidden'
            ? 'visible'
            : 'hidden';
        ...
        if (
          (obj.type === 'bar' || obj.type === 'bipolar' || obj.type === 'hbar') &&
      ...
      var defaults = {
        ...
        marker: true,
        color: co.fillStyle,
        bounding: { enabled: false, fill: 'rgba(255,255,255,0.7)', stroke: '#666' }
      };
      ...
      if (!RG.text2.domNodeCache[obj.id] || !RG.text2.domNodeCache[obj.id][cacheKey]) {
        ...
        if (
          (typeof obj.properties['chart.text.accessible.pointerevents'] === 'undefined' ||
  ...
  RG.att = RG.attribution = function(obj) {
    ...
    var width = ca.width,
      ...
      href =
        typeof prop['chart.attribution.href'] === 'string'
          ? prop['chart.attribution.href']
          : 'http://www.rgraph.net/canvas/index.html';
    ...
    for (var i = 0, len = p.length; i < len; i += 1) {
      switch (p[i]) {
        ...
        case 'c':
          ...
          break;
        case 'm':
          co.moveTo(parseFloat(p[i + 1]), parseFloat(p[i + 2]));
          i += 2;
          break;
        case 'l':
          co.lineTo(parseFloat(p[i + 1]), parseFloat(p[i + 2]));
          i += 2;
          break;
        case 's':
          if (p[i + 1]) co.strokeStyle = p[i + 1];
          co.stroke();
          i++;
          break;
        case 'f':
          ...
          break;
        case 'qc':
          co.quadraticCurveTo(parseFloat(p[i + 1]), parseFloat(p[i + 2]), parseFloat(p[i + 3]), parseFloat(p[i + 4]));
          i += 4;
          break;
        case 'bc':
          co.bezierCurveTo(
          ...
          break;
        case 'r':
          co.rect(parseFloat(p[i + 1]), parseFloat(p[i + 2]), parseFloat(p[i + 3]), parseFloat(p[i + 4]));
          i += 4;
          break;
        case 'a':
          co.arc(
            ...
            parseFloat(p[i + 4]),
            parseFloat(p[i + 5]),
            p[i + 6] === 'true' || p[i + 6] === true || p[i + 6] === 1 || p[i + 6] === '1' ? true : false
          );
          i += 6;
          break;
        case 'at':
        ...
        case 'lc':
          ...
          break;
        case 'sc':
          co.shadowColor = p[i + 1];
          i++;
          break;
        case 'sb':
          co.shadowBlur = parseFloat(p[i + 1]);
          i++;
          break;
        case 'sx':
          co.shadowOffsetX = parseFloat(p[i + 1]);
          i++;
          break;
        case 'sy':
          co.shadowOffsetY = parseFloat(p[i + 1]);
          i++;
          break;
        case 'fs':
          co.fillStyle = p[i + 1];
          ...
          break;
        case 'ss':
          co.strokeStyle = p[i + 1];
          i++;
          break;
        case 'fr':
          co.fillRect(parseFloat(p[i + 1]), parseFloat(p[i + 2]), parseFloat(p[i + 3]), parseFloat(p[i + 4]));
          i += 4;
          break;
        case 'sr':
          co.strokeRect(parseFloat(p[i + 1]), parseFloat(p[i + 2]), parseFloat(p[i + 3]), parseFloat(p[i + 4]));
          i += 4;
          break;
        case 'cl':
          co.clip();
        ...
        case 'rs':
          ...
          break;
        case 'tr':
          co.translate(parseFloat(p[i + 1]), parseFloat(p[i + 2]));
          i += 2;
          break;
        case 'sl':
          co.scale(parseFloat(p[i + 1]), parseFloat(p[i + 2]));
          i += 2;
          break;
        case 'ro':
          co.rotate(parseFloat(p[i + 1]));
          i++;
          break;
        case 'tf':
          co.transform(
        ...
        case 'stf':
          ...
          break;
        case 'cr':
          co.clearRect(parseFloat(p[i + 1]), parseFloat(p[i + 2]), parseFloat(p[i + 3]), parseFloat(p[i + 4]));
          i += 4;
          break;
        case 'ld':
          var parts = p[i + 1];
          co.setLineDash(parts);
          i += 1;
          break;
        case 'ldo':
          co.lineDashOffset = p[i + 1];
          i++;
          break;
        case 'fo':
          co.font = p[i + 1];
          ...
          break;
        case 'ft':
          co.fillText(p[i + 1], parseFloat(p[i + 2]), parseFloat(p[i + 3]));
          i += 3;
          break;
        case 'st':
          co.strokeText(p[i + 1], parseFloat(p[i + 2]), parseFloat(p[i + 3]));
          i += 3;
          break;
        case 'ta':
          co.textAlign = p[i + 1];
          ...
          break;
        case 'tbl':
          co.textBaseline = p[i + 1];
          i++;
          break;
        case 'ga':
          co.globalAlpha = parseFloat(p[i + 1]);
          i++;
          break;
        case 'gco':
          co.globalCompositeOperation = p[i + 1];
          i++;
          break;
        case 'fu':
          p[i + 1](co.canvas.__object__);
          i++;
          break;
        case '':
          break;
